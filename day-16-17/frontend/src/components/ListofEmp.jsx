...
function ListOfEmps() {
  ...
  const deleteEmpById = async (id) => {
    let res = await axios.delete(`https://atpweek6.onrender.com/employee-api/employees/${id}`);
    if (res.status === 200) {
      getEmps(); 
    }
  };
  ...
  const getEmps = async () => {
    let res = await axios.get("https://atpweek6.onrender.com/employee-api/employees");
    if (res.status === 200) {
      let resObj = res.data;
